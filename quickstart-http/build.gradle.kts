...
tasks {
    create("runQuickstartHttp", RunQalipsis::class.java) {
        scenarios("quickstart-http")
    }
    ...
    create("runQuickstartHttpForCheck", RunQalipsis::class.java) {
        scenarios("quickstart-http")
        configuration(
            "minions.count" to "1000",
            "report.export.console.enabled" to "true",
            "report.export.junit.enabled" to "true",
            "report.export.junit.folder" to project.layout.buildDirectory.dir("test-results").get().asFile.path
        )
    ...
    named("check") {
        dependsOn("runQuickstartHttpForCheck")
    }
}
