...
description = "Redis. Show how to do save & poll and produce & consume"
...
task<JavaExec>("runCampaignForSaveAndPoll") {
    group = "application"
    description = "Start a campaign for the redis-save-and-poll scenario"
    mainClass.set("io.qalipsis.runtime.Qalipsis")
    maxHeapSize = "256m"
    args("--autostart", "-c", "report.export.console.enabled=true", "-s", "redis-save-and-poll")
...
task<JavaExec>("runCampaignProduceAndConsume") {
    ...
    mainClass.set("io.qalipsis.runtime.Qalipsis")
    maxHeapSize = "256m"
    args("--autostart", "-c", "report.export.console.enabled=true", "-s", "redis-produce-and-consume")
    workingDir = projectDir
    classpath = sourceSets["main"].runtimeClasspath
}
