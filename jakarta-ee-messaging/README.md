...
1. _Produce and consume from queue_:  Records are produced to the queue, then consumed periodically from the same queue
   and compared to the original saved record.
2. _Produce and consume from topic_:  Records are produced to the topic, then consumed periodically from the same topic
   and compared to the original saved record.
