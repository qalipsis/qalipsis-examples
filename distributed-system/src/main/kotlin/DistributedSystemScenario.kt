...
import io.qalipsis.plugins.kafka.consumer.KafkaConsumerRecord
...
class DistributedSystemScenario(
    ...
    fun myScenario(
        ...
        @Property(name = "redis.uri") redisUri: String,
        ...
        scenario("distributed-system") {
            ...
            .map { result ->
                ...
                ) to result.response!!.cookies["SESSION"]!!
            }
    ...
    private fun StepSpecification<*, Pair<DeviceState, Cookie>, *>.pushDataAndVerify(
        ...
        .verify { result ->
            assertThat(result).all {
                prop(RequestResult<*, HttpResponse<String>, *>::response).isNotNull().all {
                    prop(HttpResponse<*>::status).isEqualTo(HttpResponseStatus.OK)
                }
                prop(RequestResult<*, HttpResponse<String>, *>::meters).all {
    ...
    private fun StepSpecification<RequestResult<Pair<DeviceState, Cookie>, HttpResponse<String>, *>, DeviceState, *>.verifyElasticsearch(
        ...
        innerJoin(
            ...
            having = { correlationRecord ->
                correlationRecord.value.value.deviceId + ":" + correlationRecord.value.value.timestamp
            }
        ).configure {
    ...
    private fun StepSpecification<RequestResult<Pair<DeviceState, Cookie>, HttpResponse<String>, *>, DeviceState, *>.verifyKafka(
        ...
        innerJoin(
            ...
            .execute { context: StepContext<Pair<DeviceState, KafkaConsumerRecord<ByteArray?, DeviceState?>>?, Pair<DeviceState, KafkaConsumerRecord<*, DeviceState?>>?> ->
                val input = context.receive()
                val (deviceState, kafkaRecord) = input!!
                eventsLogger.debug(
                    "time-to-kafka",
                    Duration.ofMillis(kafkaRecord.receivedTimestamp - deviceState.timestamp),
                    tags = context.toEventTags()
                )
                context.send(input)
            }
            .verify {
                val (deviceState, kafkaRecord) = it!!
                // The device state received from Kafka should be similar to the one sent by HTTP.
                assertThat(kafkaRecord.value).isNotNull().isDataClassEqualTo(deviceState)
                // The device state should be received by Kafka in the next 5 seconds after its push to the HTTP server.
                assertThat(kafkaRecord.receivedTimestamp - deviceState.timestamp).isLessThan(5000)
            }.configure {
